...
class PicazzoExample5(Structure):
    ...
    def define_elements(self, elems):            
        ...
        layout.add(align, west_fibcoups = [STANDARD_GRATING_1550_TM()]) # with 1550-TM fiber couplers on the left side
               
        # 2x2 mmi        
        MMI = ShallowMmi1x2Tapered(width = 3, length = 5, wg_offset = 0.5, taper_width = 1.0)
        layout += MMI # standard fibcoups
        layout.add(MMI, west_fibcoups =  [STANDARD_GRATING_1300_TE()], east_fibcoups = [STANDARD_GRATING_1550_TM()]) # with 1300 fiber couplers on the left side, and TM on the right side
        
        layout.add(MMI, west_fibcoups = [STANDARD_GRATING_1550_TE(), STANDARD_GRATING_1300_TE()]) # on the left side, the bottom fiber coupler is 1550, the second one 1300.
