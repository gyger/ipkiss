...
# Example: Adding a mask layout to a Structure
#
...
class RingResonator(Structure):
    ...
    def define_elements(self, elems):
        ...
        # Add the layout elements based on those shapes: two paths on 
        # the same layer, with the right width
        elems += Path(layer = TECH.PPLAYER.WG.LINE, shape = shape_ring, line_width = self.ring_wg_width)
        elems += Path(layer = TECH.PPLAYER.WG.LINE, shape = shape_bus, line_width = self.bus_wg_width)
        return elems
