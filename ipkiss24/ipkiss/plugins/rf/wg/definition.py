...
class SingleShapeWaveguideElement(__BaseWaveguideElement__):
    ...
    def define_ports(self, ports):
        from ipkiss.plugins.microwave.port.port import InMicrowavePort, OutMicrowavePort
        if len(self.shape) == 0:
            self.ports = []
            return
        ports += [InMicrowavePort (position = self.shape[0], angle = angle_deg(self.shape[0],  self.shape[1]), wg_definition = self.definition()), 
                  OutMicrowavePort(position = self.shape[-1], angle = angle_deg(self.shape[-1], self.shape[-2]), wg_definition = self.definition())]
        return ports
